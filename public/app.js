...
function getResults(){
  ...
  $.getJSON('/all', function(data) {
    ...
    for (var i = 0; i<data.length; i++){
      // ...populate #results with a p-tag that includes the note's articles
      // and object id.
      $('#results').prepend('<p class="dataentry" data-id=' +data[i]._id+ '><span class="dataTitle" data-id=' +data[i]._id+ '>' + data[i].articles + '</span><span class=deleter>X</span></p>');
    }
  });
}
...
$(document).on('click', "#makenew", function(){
  ...
  // add the articles and a delete button for the note to the page
  .done(function(data){
    // add the articles and delete button to the #results section
    $('#results').prepend('<p class="dataentry" data-id=' + data._id + '><span class="dataTitle" data-id=' +data._id+ '>' + data.articles + '</span><span class=deleter>X</span></p>');
    // clear the note and articles inputs on the page
    $('#note').val("");
    $('#articles').val("");
  }
  );
...
$(document).on('click', '.deleter', function(){
  ...
  $.ajax({
    ...
    success: function(response){
      ...
      selected.remove();
      // clear the note and articles inputs
      $('#note').val("");
      $('#articles').val("");
...
// when user click's on note articles, show the note, and allow for updates
...
$(document).on('click', '#updater', function(){
  ...
  // which is linked to the specific note articles 
